...
const register = async (page: Page, email: string, password: string) => {
  try {
    await page.goto('https://d8flight.com/', { waitUntil: 'load', timeout: 60000 });
    await clearOverlays(page);
    await page.goto('https://d8flight.com/page-login', { waitUntil: 'load', timeout: 60000 });
    await clearOverlays(page);
    // Click on register link if available
    ...
    await fillField(page, 'input[type="email"], input[name*="email" i]', email);
    await fillField(page, 'input[type="password"], input[name*="password" i]', password);
    // Submit registration
    const registerBtn = page.getByRole('button', { name: /register|sign up|create account/i }).first();
    if (await registerBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await registerBtn.click({ force: true });
      await page.waitForTimeout(3000);
      await clearOverlays(page);
      // Handle YES button if it appears
      const yesButton = page.getByRole('button', { name: 'YES', exact: true }).or(page.getByText('YES', { exact: true })).first();
      if (await yesButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        await yesButton.click({ force: true });
        await page.waitForTimeout(2000);
      }
      await expect(page.locator('body')).toContainText(/account|dashboard|logout|registration successful|created/i, { timeout: 15000 });
    } else {
      console.log('Registration button not found, user may already be logged in');
    }
  } catch (error) {
