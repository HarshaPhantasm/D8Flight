...
test('15.1 Complete Purchase Flow', async ({ page }) => {
  ...
  const productName = page
    .locator(
      'h1, .product-title, .title-detail').first();
  await expect(productName).toBeVisible();
  // ---------------- SELECT GMO CAKE ----------------
  console.log('Selecting Flavor');
  const flavorBtn = page
    .locator(
      'button, a, .variation-button, .swatch-anchor').filter({hasText: /GMO CAKE/i}).first();
  ...
  if (await qtyPlus.isVisible().catch(() => false)) {
    ...
    const qtyInput = page.locator('input[type="number"], input.qty').first();
    await qtyInput.fill('2');
  }
  // ---------------- ADD TO CART ----------------
  console.log('Adding Product To Cart');
  const addToCartBtn = page
    .locator('button.button-add-to-cart, button:has-text("ADD TO CART"), button:has-text("Add to cart")').first();
  ...
  if (await cardNumber.isVisible({ timeout: 5000 }).catch(() => false)) {
    await cardNumber.fill('378282246310005');
    const expiry = cardFrame.locator('input[name*="exp" i], input[placeholder*="MM/YY" i]').first();
    await expiry.fill('12/27');
    const cvv = cardFrame.locator('input[name*="cvv" i], input[placeholder*="cvv" i]').first();
    await cvv.fill('1234');
    const postal = cardFrame.locator('input[name*="postal" i], input[placeholder*="ZIP" i]').first();
    if (await postal.isVisible({ timeout: 3000 }).catch(() => false)) {
      await postal.fill('60436');
    }
  } else {
    // Fallback to direct page selectors
    await page.getByPlaceholder('Card number').fill('378282246310005');
    await page.getByPlaceholder('MM / YY').fill('12/27');
    await page.getByPlaceholder('CVC').fill('1234');
    await page.getByPlaceholder('Zip').fill('60436');
  }
...
await expect(page.locator('body')).toContainText(/coupon|discount|WELCOME10/i);
  ...
  await clearOverlays(page);
  const placeOrderBtn = page.locator('#place_order, button:has-text("Place Order"), button:has-text("place order"), button:has-text("PLACE ORDER")').first();
  if (await placeOrderBtn.isVisible({ timeout: 10000 }).catch(() => false)) {
    await placeOrderBtn.click({ force: true });
  } else {
    // Try alternative selector
    const altPlaceOrderBtn = page.locator('button').filter({ hasText: /place order/i }).first();
    if (await altPlaceOrderBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await altPlaceOrderBtn.click({ force: true });
    }
  }
